...
try {
   ...
   /********************************/
    ...
    $param = new SoapParam(
        array(
            "recordIncoming"  => "Always",
	    "recordOutgoing"  => "Never"
        ),
        'setCallMonitor'
    );

    $resultado = $client->setCallMonitor($param);

    if($resultado->return)
      echo "Call Monitor configurated";
    else
      echo "Call Monitor could not be configurated";
}
catch (SoapFault $e) {
