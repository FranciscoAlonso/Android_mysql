...
(function($) {
    jQuery.SOAPClient = function(options) {
            ...
            this._sendSoapRequest = function()
            {
                ...
                try{
                    ...
                    // build SOAP request
                        ...
                        oBuffer.push("</"+this.settings.methode+">");
                        oBuffer.push("</soap:Body>");
                        oBuffer.push("</soap:Envelope>");           
                        ...
                        jQuery.ajax({
                                ...
                                success     : function(xmlEnString, statut,xmlHttp){
                                                        this._onSendSoapRequest(this.settings.methode, this.settings.async, this.settings.success, this.settings.wsdl, xmlHttp, xmlEnString);
                                                        if(this.settings.spinner){
                    ...
                    var txt = "There was an error with the connection to the web service.\n\n";
                    txt    += "Click OK to return to the index page.";
                    alert(txt);
                   // DO SOMETHING
                }
            }.bind(this)
        ...
        this._onSendSoapRequest = function(method, async, callback, wsdl, req, xmlString)
        {        
            var o = null;
            var is_error=false;
            ...
            {
                var nd = this._getElementsByTagName(req.responseXML,"SOAP-ENV:Body");
                if(nd.length == 0)
                {
            ...
            {   
                ...
                {
                    callback(retour, req.responseXML, xmlString);
                }
                if(typeof(callback)=="object")
                {
                    if(is_error)
                        callback.error(retour, req.responseXML, xmlString);
                    else
                        callback.success(retour, req.responseXML, xmlString);
                }
            }
            if(!async)
                return retour;
